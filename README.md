...
Pour une installation manuelle, arrêter PM2 hors mise à jour, sauvegarder les données de production, installer le nouveau code et ecosystem.config.js, puis recréer le service PM2 en production. Lors d'une mise à jour lancée par l'ancien worker, le nouveau serveur continue temporairement à utiliser data : la migration est reportée au prochain démarrage sans verrou.

Au démarrage sans verrou, le serveur déplace automatiquement l'ancien dossier data vers backend/data et .deploy vers backups/updates. Un DATA_DIR pointant vers l'ancien dossier data du projet est également migré ; un DATA_DIR externe personnalisé reste inchangé. Si les deux dossiers de données contiennent des fichiers, le serveur s'arrête sans fusionner ni écraser : les données doivent être rapprochées manuellement. Avec data/update.lock, le serveur démarre sur data sans déplacer le verrou, les images ou release.json. Après succès et suppression du verrou par le worker, un redémarrage normal termine la migration. Ne pas retirer un verrou manuellement sans vérifier la transaction : une récupération non résolue doit rester protégée.
